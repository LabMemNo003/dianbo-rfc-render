...
Markup.url = (text) => {
    ...
    // Match general URI
    text = text.replace(/http:\/\/\S+.html/g, (uri) => T.enc(T.uri, uri));
    // Match general URI across two lines
    text = text.replace(/(http:\/\/\S+)(\n[ \t]*)(\S+.html)/g, (_, p1, mid, p2) => T.enc(T.uri, p1 + p2, p1) + mid + T.enc(T.uri, p1 + p2, p2));
