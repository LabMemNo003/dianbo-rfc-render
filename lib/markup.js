...
Markup.header = (text) => {
    ...
    return text.replace(/(?<=\n\f.*\n).*(?=\n)/g, (mat) => {
        ...
        mat = mat.replace(/RFC (\d+)/, (mat, ind) => T.enc(T.rfc, ind, mat, 'gray'));
...
Markup.url = (text) => {
    ...
    text = text.replace(/(?<=<(URL: )?)http:\/\/\S+(?=>)/g, (uri) => {
        if (Markup.debugged) console.log(uri);
        return T.enc(T.uri, uri)
    });
    // Match the URI end with .html
    text = text.replace(/http:\/\/\S+.html/g, (uri) => {
        if (Markup.debugged) console.log(uri);
        return T.enc(T.uri, uri)
    });
    // Match the URI end with .html and across two lines
    text = text.replace(/(http:\/\/\S+)(\n[ \t]*)(\S+.html)/g, (uri, p1, mid, p2) => {
        if (Markup.debugged) console.log(uri);
    ...
    text = text.replace(/http:\/\/\S+[^.\s]/g, (uri) => {
        if (Markup.debugged) console.log(uri);
        return T.enc(T.uri, uri)
    });
