...
Markup.do = (text) => {
    ...
    text = Markup.refRfc(text);
    text = Markup.refBcp(text);
...
Markup.table = Markup.tab = (text) => {
    ...
    return text.replace(/Table of Contents\n\n(([ \t]+.*(\n.*)?\d+\n)+(\n{3,}\S.+\n\f\n\S.+\n\n\n)?)+/, (tab) => {
        if (Markup.debug.tab) console.log(tab);

        // Match '   1.1  Introduction .................... 10'
        // Tip: In RFC 7322, page 2, it says ' 4.8.6.6. Referencing Other Standards Development\n  Organizations (SDOs) ......................18'
        tab = tab.replace(/(?<=^[ \t]+)(\d+(?:\.\d+)*)(.*?\n?.*?)(\d+)$/mg, (_, sind, mid, pind) => {
            return T.enc(T.to_sec, sind, sind) + mid + T.enc(T.to_page, pind);
        });

        // Match '  Appendix A. Acknowledgements ...................37'
        // Match '    A.2. Returning an I-D to the Document Stream ....................23'
        ...
        // Tip: In RFC 7322, page 3, it says '  Authors' Addresses ................................................24'
...
Markup.referenceSection = Markup.refSec = (text) => {
    ...
    return text.replace(/(?<=\n)(\d+(?:\.\d+)*\.? +.*References\n\n)(( +\[\S+\]\n?(( +[^\[\s].*\n)( +\S.*\n)*\n)+)|(\n*\S.*\n\f.*\n\S.*\n*))+/g, refSec => {
        ...
        return refSec.replace(/(?<=\n +\[)(\S+)(\]\n? +)((?: *\S.*\n)+)/g, (_, ind, mid, ref) => {
            ...
            const inner = Markup.rfc(Markup.bcp(ind));
            return T.enc(T.ref, ind, inner) + mid + ref;
        });
    });
};

// Markup reference which refer to a RFC
Markup.refRfc = (text) => {
    // Match 'Section 1.1 of [RFC0000]'
    text = text.replace(/section (\d+(?:\.\d+)*) of \[RFC(\d+)\]/ig, (mat, sind, rind) => {
        let title = Markup._refTitles[`RFC${rind}`];
        if (title === undefined) title = '';
        return T.enc(T.rfc_sec, rind, sind, mat, title);
    });
    // Match 'Section\n 1.1\n of\n [RFC0000]'
    text = text.replace(/section(?:\n *| )(\d+(?:\.\d+)*)(?:\n *| )of(?:\n *| )\[RFC(\d+)\]/ig, (mat, sind, rind) => {
        let title = Markup._refTitles[`RFC${rind}`];
        if (title === undefined) title = '';
        return mat.replace(/(.*)(\n *)(.*)/g, (_, p1, mid, p2) => T.enc(T.rfc_sec, rind, sind, p1, title) + mid + T.enc(T.rfc_sec, rind, sind, p2, title));
    });
    // Match '[RFC0000], Section 1.0'
    // Tip: In RFC 2616, page 64, it says '[RFC 2068], section 14.19'
    text = text.replace(/\[RFC ?(\d+)\], Section (\d+(?:\.\d+)*)/ig, (mat, rind, sind) => {
        let title = Markup._refTitles[`RFC${rind}`];
        if (title === undefined) title = '';
        return T.enc(T.rfc_sec, rind, sind, mat, title);
    });
    ...
    text = text.replace(/\[RFC(\d+)\], Appendix ([A-Z](\.\d)*)/g, (mat, rind, aind) => {
        let title = Markup._refTitles[`RFC${rind}`];
        if (title === undefined) title = '';
        return T.enc(T.rfc_appx, rind, aind, mat, title);
    });
    // Match '[RFC0000]'
    text = text.replace(/(?<=\[)RFC(\d+)(?=\])/g, (mat, rind) => {
        let title = Markup._refTitles[`RFC${rind}`];
        if (title === undefined) title = '';
        return T.enc(T.rfc, rind, mat, title);
    });

    return text;
};

// Markup reference which refer to a BCP
Markup.refBcp = (text) => {
    // Match '[BCP0000], Section 1.0'
    // Tip: In RFC 7322, page 13, it says '[BCP18], Section 6'
    text = text.replace(/\[BCP(\d+)\], Section (\d+(?:\.\d+)*)/ig, (mat, bind, sind) => {
        let title = Markup._refTitles[`BCP${bind}`];
        if (title === undefined) title = '';
        return T.enc(T.bcp_sec, bind, sind, mat, title);
    });
    // Match '[BCP0000],\n Section\n 1.0'
    text = text.replace(/\[BCP(\d+)\],(?:\n *| )Section(?:\n *| )(\d+(?:\.\d+)*)/ig, (mat, bind, sind) => {
        let title = Markup._refTitles[`BCP${bind}`];
        if (title === undefined) title = '';
        return mat.replace(/(.*)(\n *)(.*)/g, (_, p1, mid, p2) => T.enc(T.bcp_sec, bind, sind, p1, title) + mid + T.enc(T.bcp_sec, bind, sind, p2, title));
    });
    // Match '[BCP0000]'
    text = text.replace(/(?<=\[)BCP(\d+)(?=\])/g, (mat, bind) => {
        let title = Markup._refTitles[`BCP${bind}`];
        if (title === undefined) title = '';
        return T.enc(T.rfc, bind, mat, title);
    });

    return text;
};

// Markup rest reference (must be called after Markup.refSec())
Markup.reference = Markup.ref = (text) => {
    ...
    for (let ind in Markup._refTitles) {
        ...
        const title = Markup._refTitles[ind];
        text = text.replace(RegExp(`(?<=\\\[)${ind}(?=\\\])`, 'g'), T.enc(T.to_ref, ind, title));
    }
